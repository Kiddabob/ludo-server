...
// On Render, PORT is provided by the platform.
...
// random UUID that works on older Node too
const uid = () =>
...
function canLeaveBase(room, color) {
  ...
  return sameColorCount < 2; // cannot enter if your own blockade
}

function captureIfAllowed(room, color, destP) {
  if (destP >= 52) return;          // in lane: no capture
  const ringIdx = onRingIndex(color, destP);
  if (SAFE_TILES.has(ringIdx)) return; // safe tile
...
function tryMove(room, player, tokenIdx) {
  ...
  if (tok.t === "base") {
    ...
    tok.t = "path";
    tok.p = 0; // enter at your start tile
...
// Very simple bot: leave base on 6 > finish if possible > first legal
function botPickMove(room, color) {
  const toks = room.tokens[color] || [];
  if (room.dice === 6) {
    const idx = toks.findIndex(t => t.t === "base");
    if (idx !== -1 && canLeaveBase(room, color)) return idx;
  }
...
function maybeDriveBot(room) {
  ...
  (async () => {
    await wait(350);
    if (room.dice == null) { doRoll(room, p); await wait(300); }
    if (room.status !== "playing") return;
...
wss.on("connection", (ws) => {
  ws.on("message", (buf) => {
    ...
    if (type === "move") { const i = Number(msg.tokenIdx); if (Number.isInteger(i)) doMove(room, player, i); return; }
